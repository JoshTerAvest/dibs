...
function formatAction(row) {
  ...
  switch (a) {
    ...
    case 'left_click_drag':
      return `drag [${(input.start_coordinate || []).join(',')}]→[${(input.coordinate || []).join(',')}]`;
    case 'mouse_move':
      return `move ${coord}`;
    case 'scroll':
      return `scroll ${input.scroll_direction || '?'} x${input.scroll_amount || '?'}${coord ? ' ' + coord : ''}`;
    case 'type':
      return `type "${truncate(input.text, 40)}"`;
    case 'key':
      return `key ${input.text || ''}${input.repeat > 1 ? ' x' + input.repeat : ''}`;
    case 'hold_key':
      return `hold ${input.text || ''} ${input.duration || ''}s`;
    case 'wait':
      return `wait ${input.duration || ''}s`;
    case 'zoom':
      return `zoom [${(input.region || []).join(',')}]`;
    case 'focus_window':
      return `focus ${input.title || input.hwnd || ''}`;
    case 'set_clipboard':
      return `set_clipboard "${truncate(input.text, 40)}"`;
    case 'launch':
      return `launch ${truncate(input.command, 40)}`;
    default:
      return ''; // read-only, no params (screenshot, cursor_position, …) — Action column already names it
  }
}
...
function deriveDashboardState(state) {
  ...
  if (paused && HUMAN_PAUSE_REASONS.has(reason))
    return { name: 'human', label: 'You have the desk' };
  if (paused) return { name: 'paused', label: 'Paused' + (reason ? ` (${reason})` : '') };
  if (pending)
    return {
      name: 'consent',
      label: `${pending.name || pending.agent_id || 'an agent'} is asking…`,
    };
  if (holder)
    return { name: 'agent', label: `${holder.name || holder.agent_id || 'an agent'} has dibs` };
...
function renderDesk(state) {
  ...
  queueEl.innerHTML = queue.length
    ? queue
        .map(
          (q) =>
            `<li>${escapeHtml(q.name)} <span class="meta">${escapeHtml(q.agent_id)} &middot; ${relSpan(q.since, 'waiting ')}</span></li>`
        )
        .join('')
...
function updateConsentBar() {
  ...
  if (!bar) return;
  if (!consentExpiresAt || !consentRequestedAt) {
    bar.style.width = '0%';
    return;
  }
...
function renderConsent(state) {
  ...
  if (pending) {
    ...
    wrap.hidden = false;
    document.getElementById('consent-agent').textContent =
      pending.name || pending.agent_id || 'an agent';
  ...
  recentEl.innerHTML = recent.length
    ? recent
        ...
        .map(
          (r) =>
            `<li>${escapeHtml(r.agent_id)} <span class="decision decision-${escapeHtml(r.decision)}">${escapeHtml(r.decision)}</span> <span class="meta">${relSpan(r.at)}</span></li>`
        )
        .join('')
...
document
  .getElementById('btn-consent-allow')
  .addEventListener('click', () => decideConsent('allow'));
  ...
  body.innerHTML = agents
    .map((a) => {
      ...
      const badges = [
        ...
        win
          ? `<span class="badge badge-consent">consent ${formatCountdown(win.consent_until)}</span>`
          : '',
      ...
      const action = a.revoked
        ? ''
        : `<button class="btn btn-small ${pending ? 'btn-confirm' : 'btn-outline'}" data-revoke="${escapeHtml(a.agent_id)}">${pending ? 'Confirm?' : 'Revoke'}</button>`;
      return `<tr>
...
function renderScreenSelect(display) {
  ...
  if (currentScreen == null)
    currentScreen = (display && display.default_screen) ?? (screens[0] ? screens[0].index : 0);
  const key = screens.map((s) => s.index).join(',');
  if (sel.dataset.key !== key) {
    sel.innerHTML = screens
      .map(
        (s) =>
          `<option value="${s.index}">screen ${s.index}${s.primary ? ' (primary)' : ''} — ${s.width}×${s.height}</option>`
      )
      .join('');
    sel.dataset.key = key;
  ...
  document.getElementById('screenshot-meta').textContent = shot
    ? `${shot.width}×${shot.height} @ ${shot.scale}x`
    : '';
}
  ...
  for (const img of body.querySelectorAll('img[data-shot]')) {
    ...
    try {
      ...
      img.addEventListener('click', async () => {
        try {
          window.open(await apiBlobUrl(path), '_blank');
        } catch (_) {
          /* already toasted */
        }
      });
