...
class OverlayBase:
    ...
    def set_holder(
        self, name: str | None, purpose: str = "", expires_at: str | None = None
    ) -> None:
        pass
    ...
    def flash_click(self, x: int, y: int, button: str = "left") -> None:
        pass

    def show_typing(self, active: bool) -> None:
        pass

    def show_human(self, seconds: float = 2.0) -> None:
        pass
    ...
    def dismiss_consent(self, request_id: str) -> None:
        pass
...
class NullOverlay(OverlayBase):
    ...
    def set_holder(self, name, purpose="", expires_at=None):
        self._rec("set_holder", name, purpose, expires_at)

    def set_paused(self, reason):
        self._rec("set_paused", reason)

    def flash_click(self, x, y, button="left"):
        self._rec("flash_click", x, y, button)

    def show_typing(self, active):
        self._rec("show_typing", active)

    def show_human(self, seconds=2.0):
        self._rec("show_human", seconds)
    ...
    def dismiss_consent(self, request_id):
        self._rec("dismiss_consent", request_id)
...
def _get_monitors():
    ...
    MonitorEnumProc = ctypes.WINFUNCTYPE(
        ctypes.c_int, wintypes.HANDLE, wintypes.HANDLE, ctypes.POINTER(RECT), wintypes.LPARAM
    )
...
def _create_font(size: int, bold=False):
    ...
    font_path = os.path.join(
        os.environ["WINDIR"], "Fonts", "segoeuib.ttf" if bold else "segoeui.ttf"
    )
    try:
...
class Overlay(OverlayBase):
    ...
    def set_holder(self, name, purpose="", expires_at=None):
        self._post(self._impl_set_holder, name, purpose, expires_at)

    def set_paused(self, reason):
        self._post(self._impl_set_paused, reason)

    def flash_click(self, x, y, button="left"):
        self._post(self._impl_flash_click, int(x), int(y))

    def show_typing(self, active):
        self._post(self._impl_show_typing, bool(active))

    def show_human(self, seconds=2.0):
        self._post(self._impl_show_human, float(seconds))

    def prompt_consent(self, request_id, name, purpose, timeout_s, on_decision):
        self._post(
            self._impl_prompt_consent, request_id, name, purpose, float(timeout_s), on_decision
        )

    def dismiss_consent(self, request_id):
        self._post(self._impl_dismiss_consent, request_id)
    ...
    def _run(self) -> None:
        try:
            ...
            self._hwnd_msg = _user32.CreateWindowExW(
                0,
                class_name,
                ctypes.c_wchar_p("Msg"),
                0,
                0,
                0,
                0,
            ...
            self.available = False
            if self._hwnd_msg:
                _user32.DestroyWindow(self._hwnd_msg)
            if self._hwnd_cursor:
        ...
        if msg == WM_TIMER:
            ...
            while True:
                ...
                try:
                    ...
                    log.exception("dibs.overlay queued fn failed")
            return 0
        ...
        def revert():
            if self._state == "human":
                self._state = (
                    ...
                    else ("active" if self._holder_name else "hidden")
                )
                self._update_all()
    ...
    def _on_consent_click(self, x, y):
        ...
        deny = st["btn_deny"]
        decided = None
        if allow and allow[0] <= x <= allow[2] and allow[1] <= y <= allow[3]:
            decided = True
        elif deny and deny[0] <= x <= deny[2] and deny[1] <= y <= deny[3]:
            decided = False
    ...
    def _get_edge_img(self, color, width, height, is_vertical, is_end):
        ...
        if key in self._edge_imgs:
            return self._edge_imgs[key]
    ...
    def _update_banner(self):
        ...
        if self._state == "active":
            ...
            p = (
                self._holder_purpose[:57] + "…"
                if len(self._holder_purpose) > 60
                else self._holder_purpose
            )
        ...
        if self._notify_text:
            draw.rounded_rectangle(
                (0, bh + 4, bw - 1, bh + 28), radius=8, fill=(*(_hex_to_rgb(bg)), 224)
            )
    ...
    def _update_consent(self):
        ...
        draw.rounded_rectangle(
            (0, 0, w - 1, h - 1), radius=8, fill=(*bg, 255), outline=(*border, 255), width=2
        )
...
def create(settings: Any) -> OverlayBase:
    ...
    try:
        return Overlay(
            halo_color=getattr(ov, "halo_color", "#00e5ff"), banner=getattr(ov, "banner", True)
        )
    except Exception as e:
