...
class FakeActions:
    ...
    def allow(self, rid):
        self.calls.append(("allow", rid))

    def deny(self, rid):
        self.calls.append(("deny", rid))
...
def test_consent_beats_agent():
    st = _state(
        lease={"holder": _holder(), "queue": []},
        consent={
            "pending": {"request_id": "r1", "name": "gemini", "purpose": "file taxes"},
            "windows": [],
            "recent": [],
        },
    )
...
def test_human_pause_beats_everything():
    st = _state(
        paused=True, pause_reason="human_took_the_mouse", lease={"holder": _holder(), "queue": []}
    )
    name, tip, detail = tray.derive_state(st)
...
def test_menu_spec_pending_and_paused():
    st = _state(
        ...
        lease={"holder": _holder(), "queue": []},
        consent={
            "pending": {"request_id": "r9", "name": "gemini", "purpose": "x"},
            "windows": [],
            "recent": [],
        },
    )
...
def test_real_tray_cycles_states():
    ...
    try:
        ...
        t.refresh()
        time.sleep(0.6)
        titles.append(t.last_title)
        actions.state = _state(
            consent={
                "pending": {"request_id": "r1", "name": "gemini", "purpose": "demo"},
                "windows": [],
                "recent": [],
            }
        )
