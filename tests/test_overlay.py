...
def test_null_overlay_records_every_call():
    ...
    ov.show_human(3.0)
    decisions = []
    ov.prompt_consent("r1", "agent-a", "wants the desk", 10.0, lambda rid, ok: decisions.append((rid, ok)))
    ov.dismiss_consent("r1")
    ov.notify("hello", 1.5)
    ov.stop()
    assert [c[0] for c in ov.calls] == [
        "start", "set_holder", "set_paused", "flash_click", "show_typing", "show_human",
        "prompt_consent", "dismiss_consent", "notify", "stop",
    ]
...
def test_create_returns_overlay_instance_when_enabled():
    settings = SimpleNamespace(overlay=SimpleNamespace(enabled=True, halo_color="#ff00ff", banner=False))
